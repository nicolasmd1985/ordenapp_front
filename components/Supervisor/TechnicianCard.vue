  ...
  <div class="h-full flex justify-center items-center p-6">
    ...
    <form @submit.prevent="submit">
      ...
      <div class="mb-4 hidden">
        <label for="id_technician" class="block text-sm font-medium text-gray-700">Id Technician</label>
        <input :value="id_technician"
        type="text" id="id_technician" name="id_technician" class="py-[0.32rem] p-3 mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
      </div>
      <button 
        type="submit" 
...
<script setup>
  ...
  const submit = async () => {
    ...
    const id_technician = document.getElementById('id_technician').value;
    ...
    if (props.type == 'create') {
      graphql('CreateTechnician', id_technician, first_name, last_name, phone, email, subsidiary_name)
    } else {
      graphql('UpdateTechnician', id_technician, first_name, last_name, phone, email, subsidiary_name)
    }
  };

  async function graphql(operation, id_technician, first_name, last_name, phone, email, subsidiary_name) {
    ...
    if (data.value && data.value.createTechnician?.errors.length == 0 || data.value.updateTechnician?.errors.length == 0) {
      router.push({ path: '/supervisor/techniciansview' });
    }
    if (error.value || data.value.createTechnician?.errors.length > 0 || data.value.updateTechnician?.errors.length > 0) {
      // useNuxtApp().$toast.error(data.value.createTechnician.errors);
      useNuxtApp().$toast.error(data.value.createTechnician?.errors || data.value.updateTechnician?.errors);

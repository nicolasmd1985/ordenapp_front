...
<script setup lang="ts">
  ...
  import useTechnician from "~/store/technician";
  ...
  const client_data = ref(props.technician_data);
  ...
  const editItem = async (option: any) => {
    ...
    if (data.value) {
      technician.id_technician = data.value.getClient.id || '';
      technician.firstName = data.value.getClient.firstName || '';
      technician.lastName = data.value.getClient.lastName || '';
      technician.phone = data.value.getClient.phoneNumber1 || '';
      technician.email = data.value.getClient.email || '';
      router.push({ path: '/supervisor/technicianview'});
    }
  };
